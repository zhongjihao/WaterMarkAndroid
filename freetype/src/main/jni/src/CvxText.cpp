//
// Created by zhongjihao on 15/09/19.
//
...
FT_Error CvxText::init(const char *font_path)
{
    ...
    LOGD("%s: init start",__FUNCTION__);

    // 打开字库文件, 创建一个字体
    ret = FT_Init_FreeType(&m_library);
    if (ret != FT_Err_Ok) {
        LOGE("%s: FT_Init_FreeType: ret = %d",__FUNCTION__, ret);
        return ret;
    }

    ret = FT_New_Face(m_library, font_path, 0, &m_face);
    if (ret != FT_Err_Ok) {
        LOGE("%s: FT_New_Face: ret = %d", __FUNCTION__,ret);
        return ret;
    }
...
{
    if (m_face == NULL) {
        LOGE("%s: setFont failed. Shouldn't be here.",__FUNCTION__);
        return;
    }
...
int CvxText::putText(cv::Mat &img, const char* text, cv::Point org,
